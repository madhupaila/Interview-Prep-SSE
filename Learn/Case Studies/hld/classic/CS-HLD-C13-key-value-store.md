# Design Key-Value Store — Case Study

**Case Study ID:** CS-HLD-C13
**Track:** Classic HLD
**Companies:** Amazon, Google
**Difficulty:** Medium
**Related question:** [Q13-key-value-store.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q13-key-value-store.md)

---

## Part 1 — Business Context

**Industry analog:** Leading products in the Design Key-Value Store domain

This case study examines **Design Key-Value Store** — a system type commonly built at Amazon and similar organizations. Design a scalable system for: **Key-Value Store**.
...
|----------|-------------|---------------------|
| Must | Core features for key-value store | Verified in integration tests |
| Must | User authentication and authorization | Verified in integration tests |
| Must | quorum consistency | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | Scale (DAU, QPS)? | 1B keys |
| 2 | Read vs write ratio? | Dynamo-style |
...
| 7 | MVP scope? | Core quorum consistency |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Key-Value Store.  
...
Design a scalable system for: **Key-Value Store**.
...
```
Client → LB → API → Redis Cache → PostgreSQL
              ↓
         Key Generation Service (counter)
```
...
**MVP success criteria for Design Key-Value Store:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Key-Value Store**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Let me clarify requirements for key-value store. We're targeting 1B keys with a Dynamo-style pattern. I'll design the MVP core flow, estimate scale, draw the architecture, and we can deep-dive where you prefer."
...
> **Deep dive:** "The critical design decision here is quorum consistency. [Explain pattern-specific logic — e.g., for feed: hybrid fan-out; for payments: idempotency keys and ledger.]"
...
| Capability | Leading products in the Design Key-Value Store domain (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q13-key-value-store.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q13-key-value-store.md)
