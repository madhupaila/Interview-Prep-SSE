# Design IoT Device Platform — Case Study

**Case Study ID:** CS-HLD-C47
**Track:** Classic HLD
**Companies:** AWS, Google
**Difficulty:** Medium
**Related question:** [Q47-iot-device-platform.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q47-iot-device-platform.md)

---

## Part 1 — Business Context

**Industry analog:** AWS IoT Core device shadow

This case study examines **Design IoT Device Platform** — a system type commonly built at AWS and similar organizations. Design a scalable system for: **IoT Device Platform**.
...
|----------|-------------|---------------------|
| Must | Core features for iot device platform | Verified in integration tests |
| Must | User authentication and authorization | Verified in integration tests |
| Must | telemetry pipeline | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | Scale (DAU, QPS)? | 100M devices |
| 2 | Read vs write ratio? | MQTT |
...
| 7 | MVP scope? | Core telemetry pipeline |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design IoT Device Platform.  
...
Design a scalable system for: **IoT Device Platform**.
...
```
Devices → MQTT Broker → Ingestion → Time-series DB → Rules/Alerts
```
...
**MVP success criteria for Design IoT Device Platform:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design IoT Device Platform**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Let me clarify requirements for iot device platform. We're targeting 100M devices with a MQTT pattern. I'll design the MVP core flow, estimate scale, draw the architecture, and we can deep-dive where you prefer."
...
> **Deep dive:** "The critical design decision here is telemetry pipeline. [Explain pattern-specific logic — e.g., for feed: hybrid fan-out; for payments: idempotency keys and ledger.]"
...
| Capability | AWS IoT Core device shadow (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q47-iot-device-platform.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q47-iot-device-platform.md)
