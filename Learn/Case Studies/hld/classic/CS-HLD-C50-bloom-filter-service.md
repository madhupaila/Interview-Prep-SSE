# Design Bloom Filter Service — Case Study

**Case Study ID:** CS-HLD-C50
**Track:** Classic HLD
**Companies:** Google, Meta
**Difficulty:** Medium
**Related question:** [Q50-bloom-filter-service.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q50-bloom-filter-service.md)

---

## Part 1 — Business Context

**Industry analog:** Chrome Safe Browsing and Cassandra BF

This case study examines **Design Bloom Filter Service** — a system type commonly built at Google and similar organizations. Design a scalable system for: **Bloom Filter Service**.
...
|----------|-------------|---------------------|
| Must | Core features for bloom filter service | Verified in integration tests |
| Must | User authentication and authorization | Verified in integration tests |
| Must | distributed bloom | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | Scale (DAU, QPS)? | dedup at scale |
| 2 | Read vs write ratio? | no false negatives |
...
| 7 | MVP scope? | Core distributed bloom |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Bloom Filter Service.  
...
| Metric | Estimate |
|--------|----------|
| Scale | dedup at scale |
| Traffic pattern | no false negatives |
...
Design a scalable system for: **Bloom Filter Service**.
...
```
Producer → Bloom Filter Service (sharded) → Consumers check membership
```
...
**MVP success criteria for Design Bloom Filter Service:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Bloom Filter Service**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Let me clarify requirements for bloom filter service. We're targeting dedup at scale with a no false negatives pattern. I'll design the MVP core flow, estimate scale, draw the architecture, and we can deep-dive where you prefer."
...
> **Deep dive:** "The critical design decision here is distributed bloom. [Explain pattern-specific logic — e.g., for feed: hybrid fan-out; for payments: idempotency keys and ledger.]"
...
| Capability | Chrome Safe Browsing and Cassandra BF (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q50-bloom-filter-service.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q50-bloom-filter-service.md)
