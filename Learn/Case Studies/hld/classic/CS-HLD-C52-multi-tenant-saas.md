# Design Multi-Tenant SaaS Platform — Case Study

**Case Study ID:** CS-HLD-C52
**Track:** Classic HLD
**Companies:** Salesforce, Atlassian
**Difficulty:** Medium
**Related question:** [Q52-multi-tenant-saas.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q52-multi-tenant-saas.md)

---

## Part 1 — Business Context

**Industry analog:** Salesforce org isolation and row-level security

This case study examines **Design Multi-Tenant SaaS Platform** — a system type commonly built at Salesforce and similar organizations. Design a scalable system for: **Multi-Tenant SaaS Platform**.
...
|----------|-------------|---------------------|
| Must | Core features for multi-tenant saas platform | Verified in integration tests |
| Must | User authentication and authorization | Verified in integration tests |
| Must | noisy neighbor | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | Scale (DAU, QPS)? | 100K tenants |
| 2 | Read vs write ratio? | tenant isolation |
...
| 7 | MVP scope? | Core noisy neighbor |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Multi-Tenant SaaS Platform.  
...
|--------|----------|
| Scale | 100K tenants |
| Traffic pattern | tenant isolation |
...
```
Tenant request → API → tenant_id isolation → Shared DB / per-tenant resources
```
...
**MVP success criteria for Design Multi-Tenant SaaS Platform:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Multi-Tenant SaaS Platform**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Let me clarify requirements for multi-tenant saas platform. We're targeting 100K tenants with a tenant isolation pattern. I'll design the MVP core flow, estimate scale, draw the architecture, and we can deep-dive where you prefer."
...
> **Deep dive:** "The critical design decision here is noisy neighbor. [Explain pattern-specific logic — e.g., for feed: hybrid fan-out; for payments: idempotency keys and ledger.]"
...
| Capability | Salesforce org isolation and row-level security (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q52-multi-tenant-saas.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q52-multi-tenant-saas.md)
