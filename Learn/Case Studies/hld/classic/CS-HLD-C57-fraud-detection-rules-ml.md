# Design Fraud Detection (Rules+ML) — Case Study

**Case Study ID:** CS-HLD-C57
**Track:** Classic HLD
**Companies:** PayPal, Stripe
**Difficulty:** Medium
**Related question:** [Q57-fraud-detection-rules-ml.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q57-fraud-detection-rules-ml.md)

---

## Part 1 — Business Context

**Industry analog:** Stripe Radar rules + ML scoring

This case study examines **Design Fraud Detection (Rules+ML)** — a system type commonly built at PayPal and similar organizations. Design a scalable system for: **Fraud Detection (Rules+ML)**.
...
|----------|-------------|---------------------|
| Must | Core features for fraud detection (rules+ml) | Verified in integration tests |
| Must | User authentication and authorization | Verified in integration tests |
| Must | feature store | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | Scale (DAU, QPS)? | 1B tx/day |
| 2 | Read vs write ratio? | rules engine |
...
| 7 | MVP scope? | Core feature store |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Fraud Detection (Rules+ML).  
...
```
Transaction → Feature Store → Rules Engine + ML Model → Decision + audit
```
...
**MVP success criteria for Design Fraud Detection (Rules+ML):** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Fraud Detection (Rules+ML)**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Let me clarify requirements for fraud detection (rules+ml). We're targeting 1B tx/day with a rules engine pattern. I'll design the MVP core flow, estimate scale, draw the architecture, and we can deep-dive where you prefer."
...
> **Deep dive:** "The critical design decision here is feature store. [Explain pattern-specific logic — e.g., for feed: hybrid fan-out; for payments: idempotency keys and ledger.]"
...
| Capability | Stripe Radar rules + ML scoring (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q57-fraud-detection-rules-ml.md](../../../System%20Design%20-%20High%20Level%20Design/03-classic-hld/questions/Q57-fraud-detection-rules-ml.md)
