# Design Embedding Service at Scale — Case Study

**Case Study ID:** CS-HLD-G06
**Track:** Gen AI / LLM HLD
**Companies:** Pinecone, Cohere, Meta
**Difficulty:** Hard
**Related question:** [Q06-embedding-service-scale.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q06-embedding-service-scale.md)

---

## Part 1 — Business Context

**Industry analog:** OpenAI embeddings API and Cohere embed

This case study examines **Design Embedding Service at Scale** — a system type commonly built at Pinecone and similar organizations. Design a production system for: **Embedding Service at Scale**.
...
|----------|-------------|---------------------|
| Must | Core user-facing capability for embedding service at scale | Verified in integration tests |
...
| Must | multi-model, GPU pooling | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | B2B multi-tenant or consumer? | Assume 100M embed requests/day unless stated |
...
| 7 | Scale? | 100M embed requests/day, batch + realtime |
| 8 | Offline/batch vs real-time? | Per focus: multi-model, GPU pooling |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Embedding Service at Scale.  
...
| Metric | Estimate |
|--------|----------|
| Scale | 100M embed requests/day |
| Throughput | batch + realtime |
...
Design a production system for: **Embedding Service at Scale**.
...
**MVP success criteria for Design Embedding Service at Scale:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Embedding Service at Scale**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Before I draw the architecture, I want to confirm scope. We're designing embedding service at scale at scale — roughly 100M embed requests/day with batch + realtime. I'll focus on the core query path and ingestion if needed, and cover safety and cost."
...
> **Deep dive:** "Key decision: multi-model, GPU pooling. For hallucination mitigation, we require citations grounded in retrieved chunks and block answers when retrieval confidence is low. Prompt injection is handled by isolating system instructions from user and document content."
...
| Capability | OpenAI embeddings API and Cohere embed (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q06-embedding-service-scale.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q06-embedding-service-scale.md)
