# Design PDF Ingestion + Citation RAG — Case Study

**Case Study ID:** CS-HLD-G11
**Track:** Gen AI / LLM HLD
**Companies:** Adobe, Harvey
**Difficulty:** Hard
**Related question:** [Q11-pdf-citation-rag.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q11-pdf-citation-rag.md)

---

## Part 1 — Business Context

**Industry analog:** Harvey AI legal RAG with precise citations

This case study examines **Design PDF Ingestion + Citation RAG** — a system type commonly built at Adobe and similar organizations. Design a production system for: **PDF Ingestion + Citation RAG**.
...
|----------|-------------|---------------------|
| Must | Core user-facing capability for pdf ingestion + citation rag | Verified in integration tests |
...
| Must | table extraction, legal docs | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | B2B multi-tenant or consumer? | Assume 50M PDFs unless stated |
...
| 7 | Scale? | 50M PDFs, citation to page |
| 8 | Offline/batch vs real-time? | Per focus: table extraction, legal docs |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design PDF Ingestion + Citation RAG.  
...
**MVP success criteria for Design PDF Ingestion + Citation RAG:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design PDF Ingestion + Citation RAG**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Before I draw the architecture, I want to confirm scope. We're designing pdf ingestion + citation rag at scale — roughly 50M PDFs with citation to page. I'll focus on the core query path and ingestion if needed, and cover safety and cost."
...
> **Deep dive:** "Key decision: table extraction, legal docs. For hallucination mitigation, we require citations grounded in retrieved chunks and block answers when retrieval confidence is low. Prompt injection is handled by isolating system instructions from user and document content."
...
| Capability | Harvey AI legal RAG with precise citations (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q11-pdf-citation-rag.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q11-pdf-citation-rag.md)
