# Design Multi-Modal Q&A (Image+Text) — Case Study

**Case Study ID:** CS-HLD-G16
**Track:** Gen AI / LLM HLD
**Companies:** Google, OpenAI
**Difficulty:** Hard
**Related question:** [Q16-multimodal-qa.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q16-multimodal-qa.md)

---

## Part 1 — Business Context

**Industry analog:** Leading products in the Design Multi-Modal Q&A (Image+Text) domain

This case study examines **Design Multi-Modal Q&A (Image+Text)** — a system type commonly built at Google and similar organizations. Design a production system for: **Multi-Modal Q&A (Image+Text)**.
...
|----------|-------------|---------------------|
| Must | Core user-facing capability for multi-modal q&a (image+text) | Verified in integration tests |
...
| Must | image upload, OCR fallback | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | B2B multi-tenant or consumer? | Assume 20M queries/day unless stated |
...
| 7 | Scale? | 20M queries/day, vision model |
| 8 | Offline/batch vs real-time? | Per focus: image upload, OCR fallback |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Multi-Modal Q&A (Image+Text).  
...
| Metric | Estimate |
|--------|----------|
| Scale | 20M queries/day |
| Throughput | vision model |
...
Design a production system for: **Multi-Modal Q&A (Image+Text)**.
...
**MVP success criteria for Design Multi-Modal Q&A (Image+Text):** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Multi-Modal Q&A (Image+Text)**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Before I draw the architecture, I want to confirm scope. We're designing multi-modal q&a (image+text) at scale — roughly 20M queries/day with vision model. I'll focus on the core query path and ingestion if needed, and cover safety and cost."
...
> **Deep dive:** "Key decision: image upload, OCR fallback. For hallucination mitigation, we require citations grounded in retrieved chunks and block answers when retrieval confidence is low. Prompt injection is handled by isolating system instructions from user and document content."
...
| Capability | Leading products in the Design Multi-Modal Q&A (Image+Text) domain (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q16-multimodal-qa.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q16-multimodal-qa.md)
