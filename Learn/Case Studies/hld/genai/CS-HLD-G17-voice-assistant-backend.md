# Design Voice Assistant Backend — Case Study

**Case Study ID:** CS-HLD-G17
**Track:** Gen AI / LLM HLD
**Companies:** Amazon Alexa, Google
**Difficulty:** Hard
**Related question:** [Q17-voice-assistant-backend.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q17-voice-assistant-backend.md)

---

## Part 1 — Business Context

**Industry analog:** Leading products in the Design Voice Assistant Backend domain

This case study examines **Design Voice Assistant Backend** — a system type commonly built at Amazon Alexa and similar organizations. Design a production system for: **Voice Assistant Backend**.
...
|----------|-------------|---------------------|
| Must | Core user-facing capability for voice assistant backend | Verified in integration tests |
...
| Must | low latency, wake word | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | B2B multi-tenant or consumer? | Assume 50M devices unless stated |
...
| 7 | Scale? | 50M devices, ASR+LLM+TTS pipeline |
| 8 | Offline/batch vs real-time? | Per focus: low latency, wake word |
| 9 | Multi-modal input? | Yes |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Voice Assistant Backend.  
...
|--------|----------|
| Scale | 50M devices |
| Throughput | ASR+LLM+TTS pipeline |
...
Design a production system for: **Voice Assistant Backend**.
...
**MVP success criteria for Design Voice Assistant Backend:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Voice Assistant Backend**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Before I draw the architecture, I want to confirm scope. We're designing voice assistant backend at scale — roughly 50M devices with ASR+LLM+TTS pipeline. I'll focus on the core query path and ingestion if needed, and cover safety and cost."
...
> **Deep dive:** "Key decision: low latency, wake word. For hallucination mitigation, we require citations grounded in retrieved chunks and block answers when retrieval confidence is low. Prompt injection is handled by isolating system instructions from user and document content."
...
| Capability | Leading products in the Design Voice Assistant Backend domain (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q17-voice-assistant-backend.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q17-voice-assistant-backend.md)
