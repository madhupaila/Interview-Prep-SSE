# Design Recommendation with LLM Rerank — Case Study

**Case Study ID:** CS-HLD-G18
**Track:** Gen AI / LLM HLD
**Companies:** Netflix, Spotify
**Difficulty:** Hard
**Related question:** [Q18-llm-recommendation-rerank.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q18-llm-recommendation-rerank.md)

---

## Part 1 — Business Context

**Industry analog:** Leading products in the Design Recommendation with LLM Rerank domain

This case study examines **Design Recommendation with LLM Rerank** — a system type commonly built at Netflix and similar organizations. Design a production system for: **Recommendation with LLM Rerank**.
...
|----------|-------------|---------------------|
| Must | Core user-facing capability for recommendation with llm rerank | Verified in integration tests |
...
| Must | latency budget 200ms | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | B2B multi-tenant or consumer? | Assume 100M users unless stated |
...
| 7 | Scale? | 100M users, candidate generation + LLM rerank |
| 8 | Offline/batch vs real-time? | Per focus: latency budget 200ms |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Recommendation with LLM Rerank.  
...
|--------|----------|
| Scale | 100M users |
| Throughput | candidate generation + LLM rerank |
...
**MVP success criteria for Design Recommendation with LLM Rerank:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Recommendation with LLM Rerank**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Before I draw the architecture, I want to confirm scope. We're designing recommendation with llm rerank at scale — roughly 100M users with candidate generation + LLM rerank. I'll focus on the core query path and ingestion if needed, and cover safety and cost."
...
> **Deep dive:** "Key decision: latency budget 200ms. For hallucination mitigation, we require citations grounded in retrieved chunks and block answers when retrieval confidence is low. Prompt injection is handled by isolating system instructions from user and document content."
...
| Capability | Leading products in the Design Recommendation with LLM Rerank domain (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q18-llm-recommendation-rerank.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q18-llm-recommendation-rerank.md)
