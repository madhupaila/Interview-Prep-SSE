# Design Chart/Diagram Generation from Text — Case Study

**Case Study ID:** CS-HLD-G31
**Track:** Gen AI / LLM HLD
**Companies:** Miro, Figma
**Difficulty:** Hard
**Related question:** [Q31-chart-diagram-generation.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q31-chart-diagram-generation.md)

---

## Part 1 — Business Context

**Industry analog:** Leading products in the Design Chart/Diagram Generation from Text domain

This case study examines **Design Chart/Diagram Generation from Text** — a system type commonly built at Miro and similar organizations. Design a production system for: **Chart/Diagram Generation from Text**.
...
|----------|-------------|---------------------|
| Must | Core user-facing capability for chart/diagram generation from text | Verified in integration tests |
...
| Must | Mermaid/JSON schema validation | Verified in integration tests |
...
|---|----------|-----------------|
| 1 | B2B multi-tenant or consumer? | Assume 5M diagrams/month unless stated |
...
| 7 | Scale? | 5M diagrams/month, structured output |
| 8 | Offline/batch vs real-time? | Per focus: Mermaid/JSON schema validation |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Design Chart/Diagram Generation from Text.  
...
| Metric | Estimate |
|--------|----------|
| Scale | 5M diagrams/month |
| Throughput | structured output |
...
Design a production system for: **Chart/Diagram Generation from Text**.
...
```
User → Orchestrator ↔ LLM ↔ Tool Registry → Sandboxed Executors
              ↓
         State Store (Redis)
```
...
**MVP success criteria for Design Chart/Diagram Generation from Text:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Design Chart/Diagram Generation from Text**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> **Opening:** "Before I draw the architecture, I want to confirm scope. We're designing chart/diagram generation from text at scale — roughly 5M diagrams/month with structured output. I'll focus on the core query path and ingestion if needed, and cover safety and cost."
...
> **Deep dive:** "Key decision: Mermaid/JSON schema validation. For hallucination mitigation, we require citations grounded in retrieved chunks and block answers when retrieval confidence is low. Prompt injection is handled by isolating system instructions from user and document content."
...
| Capability | Leading products in the Design Chart/Diagram Generation from Text domain (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q31-chart-diagram-generation.md](../../../System%20Design%20-%20High%20Level%20Design/02-genai-llm-hld/questions/Q31-chart-diagram-generation.md)
