# Table Reservation — Case Study

**Case Study ID:** CS-LLD-O51
**Track:** Classic OOD
**Companies:** OpenTable, Yelp
**Difficulty:** Medium
**Related question:** [Q51-table-reservation.md](../../../System%20Design%20-%20Low%20Level%20Design/02-classic-ood/questions/Q51-table-reservation.md)

---

## Part 1 — Business Context

**Industry analog:** Leading products in the Table Reservation domain

This case study examines **Table Reservation** — a system type commonly built at OpenTable and similar organizations. Design restaurant table booking: slots, party size, no-show policy.
...
| Must | ReservationService handles primary workflow described in requirements | Verified in integration tests |
...
- Open-Closed via AvailabilityChecker interface at variation points
...
|---|----------|-----------------|
| 1 | What is MVP scope for Table Reservation? | Core entities + 2 primary flows; extensions deferred |
| 2 | Persistence? | In-memory; Repository interface if interviewer asks |
| 3 | Multi-threaded? | Synchronize shared state if concurrent users assumed |
| 4 | Requirement: Design restaurant table booking? | Include in MVP — Design restaurant table booking |
| 5 | Requirement: party size? | Include in MVP — party size |
| 6 | Requirement: no-show policy.? | Include in MVP — no-show policy. |
...
**Status:** Accepted  
**Context:** Need to balance delivery speed, operability, and scale for Table Reservation.  
...
| `TimeSlot` | Available window |
| `Guest` | Customer |

**Nouns → classes:** `Restaurant`, `Table`, `Reservation`, `TimeSlot`, `Guest`  
...
```
┌─────────────────────┐       ┌──────────────────┐
│  ReservationService │──────>│ Strategy         │<<interface>>
│─────────────────────│       │──────────────────│
│ +orchestrate()      │       │ +apply()         │
└─────────┬───────────┘       └────────┬─────────┘
          │ owns                       │ implements
...
| Pattern | Application |
|---------|-------------|
| Strategy | Variation point in Table Reservation |

**SOLID:**
- **S:** ReservationService orchestrates; entities hold state
- **O:** New behavior via new AvailabilityChecker impl
- **D:** Depend on AvailabilityChecker interface
...
**MVP success criteria for Table Reservation:** Core flows demo-ready; p99 within 2× target; on-call runbook draft.
...
> This is a 30-minute senior loop for **Table Reservation**. Spend 5 minutes on context, 10 on HLD, 10 on LLD/boundaries, 5 on ops.

> "I'll design Table Reservation — clarify in-memory scope and MVP flows first."
>
> "Entities: `Restaurant`, `Table`, `Reservation`, `TimeSlot`, `Guest`. Domain structure separate from `ReservationService` orchestration."
>
> "Problem: Design restaurant table booking: slots, party size, no-show policy."
>
> "`Restaurant` — venue; owns its own invariants."
>
> "`Table` — capacity; owns its own invariants."
>
> "`Reservation` — booking record; owns its own invariants."
>
> "`ReservationService` validates input, coordinates entities, returns typed results."
>
...
| Capability | Leading products in the Table Reservation domain (reference) | This design (MVP) | Scale phase |
|------------|----------------------|-------------------|-------------|
...
- **Question file:** [Q51-table-reservation.md](../../../System%20Design%20-%20Low%20Level%20Design/02-classic-ood/questions/Q51-table-reservation.md)
...
- [Java implementation](../../../System Design - Low Level Design/09-code-implementations/java/classic/table-reservation/) (skeleton)
